...
public class EnviadoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Listaenviados> les = (List<Listaenviados>) Utils.fromJson(Utils.readParams(request), List.class);
        ArrayList<Listaenviados> ales= (ArrayList<Listaenviados>) les;
        for(int i=0; i<ales.size();i++){
            ServicioManager.insertListaEnviados(ales.get(i));
        }
        response.setContentType("application/json");
            response.setCharacterEncoding("UTF-8");
            response.getWriter().write(Utils.toJson(ales));
    }
