 ...
 * @author manga
 */
public class RegistroPServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
       Proveedor cli = (Proveedor) Utils.fromJson(Utils.readParams(request), Proveedor.class );
        
        Proveedor prueba = ProveedorManager.pedirProveedor(cli.getCedulap());
        response.setContentType("application/json");
        response.setCharacterEncoding("UTF-8");
        if(prueba.getClave()==null){
        
        String p = ProveedorManager.insertProveedor(cli);
